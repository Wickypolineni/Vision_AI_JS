...
let API_KEY = localStorage.getItem("apiKey"); // Use the same key as in script.js

if (API_KEY) {
  // If the API key exists, initialize live mode
  setTimeout(() => {
    setInterval(captureImage, 2000); // Wait for the camera to load
  }, 2000);
} else {
  // If the API key is not found, redirect to the main page to set it
  alert("Please provide an API key in the main UI.");
  location.href = "/";
}
